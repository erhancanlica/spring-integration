...
public class CreateEmailMessageHandler implements MessageHandler {
    private final EmailService emailService;

    @Override
    public void handleMessage(Message<?> message) throws MessagingException {
        final EmailEntity messagePayload = (EmailEntity) message.getPayload();
        log.info("The email {} has arrived to create email message handler.", messagePayload.getId());

        emailService.addToQueue(messagePayload);
    }
}